<template>
    <div v-if="isShow" v-show="isShowOfVshow" class="panel-form-item" :data-id="item.id">
        ...
        <sub-form v-else-if="item.type === 'sub-form'" v-bind="item.options" :type="item.type"
                  :id="item.id" :subs="item.subs" :model="PREVIEW.model" :is-new="isNew" :all-vars="allVars" :nodes-values-dict="nodesValuesDict" :global-disabled="!!globalDisabled" :authority="authority">
        </sub-form>
        ...
        <template v-else>
            <item :model="PREVIEW.model" :type="item.type" v-bind="item.options" :is-new="isNew" :all-vars="allVars" :nodes-values-dict="nodesValuesDict" :global-disabled="!!globalDisabled" :authority="authority"></item>
        </template>
    </div>
</template>
<script>
    ...
    export default {
        ...
        props: ["list", "item", "model", "isNew", "allVars","nodesValuesDict", "globalDisabled", "authority"],
        data() {
            return {}
        },
        computed: {
